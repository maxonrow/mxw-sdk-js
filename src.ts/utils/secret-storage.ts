...
export function decrypt(json: string, password: Arrayish, progressCallback?: ProgressCallback): Promise<SigningKey> {
    ...
    var getSigningKey = function (key: Uint8Array, reject: (error?: Error) => void) {
        ...
        if (cipher === 'aes-256-ctr') {
            ...
            computedMAC = hexlify(computeMAC(key.slice(32, 48), ciphertext)).substring(2);
        }
            ...
            computedMAC = hexlify(computeMAC(key.slice(16, 32), ciphertext)).substring(2);
        }
        else {
        ...
        if (searchPath(data, 'x-mxw/version') === '0.1') {
            ...
            var mnemonic = HDNode.entropyToMnemonic(entropy, locales[locale]);

            var node = HDNode.fromMnemonic(mnemonic, locales[locale]).derivePath(path);
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (kdf && typeof (kdf) === 'string') {
            if (kdf.toLowerCase() === 'scrypt') {
                ...
                scrypt(passwordBytes, salt, N, r, p, dkLen + 32, function (error, progress, key) {
                    if (error) {
                ...
                var key = pbkdf2(passwordBytes, salt, c, dkLen + 32, prfFunc);
...
export function encrypt(privateKey: Arrayish | SigningKey, password: Arrayish | string, options?: EncryptOptions, progressCallback?: ProgressCallback): Promise<string> {
    ...
    if (options.mnemonic) {
        if (entropy) {
            ...
            entropy = arrayify(HDNode.mnemonicToEntropy(options.mnemonic, options ? options.locale : undefined));
        }
    }
