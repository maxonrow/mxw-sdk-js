...
export const defaultPath = "m/44'/376'/0'/0/0";
...
export class HDNode {
    ...
    constructor(constructorGuard: any, privateKey: string, publicKey: string, parentFingerprint: string, chainCode: string, index: number, depth: number, mnemonic: string, path: string, wordlist?: Wordlist) {
        ...
        defineReadOnly(this, 'wordlist', wordlist);
...
export function fromExtendedKey(extendedKey: string): HDNode {
    ...
    switch (hexlify(bytes.slice(0, 4))) {
        ...
        case "0x0488b21e": case "0x043587cf":
            return new HDNode(_constructorGuard, null, hexlify(key), parentFingerprint, chainCode, index, depth, null, null, null);

        // Private Key
        case "0x0488ade4": case "0x04358394 ":
            if (key[0] !== 0) { break; }
            return new HDNode(_constructorGuard, hexlify(key.slice(1)), null, parentFingerprint, chainCode, index, depth, null, null, null);
    }
...
function _fromSeed(seed: Arrayish, mnemonic: string, wordlist?: Wordlist): HDNode {
