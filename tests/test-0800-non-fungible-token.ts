        ...
        [false, true].forEach((modifiable) => {
            [false, true].forEach((burnable) => {
                [false, true].forEach((pub) => {
                    [2, 0].forEach((endorserListLimit) => {
                        describe('Suite: NonFungibleToken - ' + JSON.stringify({ transferable, modifiable, burnable, pub, endorserListLimit }), function () {
                            this.slow(slowThreshold); // define the threshold for slow indicator
                            ...
                            it("Approve - challenge wrong fee setting", function () {
                                let overrides = {
                                    tokenFees: [
                                        ...
                                        { action: NonFungibleTokenActions.updateNFTItemMetadata, feeName: "default" }
                                    ],
                                    endorserList: [],
                                    mintLimit: 1,
                                    transferLimit: 1,
                            ...
                            it("Approve", function () {
                                ...
                                return performNonFungibleTokenStatus(nonFungibleTokenProperties.symbol, token.NonFungibleToken.approveNonFungibleToken, overrides).then((receipt) => {
                                    ...
                                    if (0 == endorserListLimit) {
                                        expect(error).have.property("code").to.eq(errors.INVALID_ARGUMENT);
                                    }
                                    else {
                                        throw error;
                                    }
                            ...
                            if (0 < endorserListLimit) {
                                ...
                                it("State", function () {
                                    return issuerNonFungibleToken.getState().then((state) => {
                                        ...
                                        expect(state).have.property("endorserListLimit").is.satisfy(function (value) { return value.eq(endorserListLimit) });
                                ...
                                it("Update endorser list - challenge endorser list limit", function () {
                                    let endorsers = [middleware.address, issuer.address, wallet.address];
                                    return issuerNonFungibleToken.updateEndorserList(endorsers).then((receipt) => {
                                        expect(receipt).is.not.exist;
                                    }).catch(error => {
                                        expect(error).have.property("code").to.eq(errors.NOT_ALLOWED);
